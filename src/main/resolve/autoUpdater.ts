...
import { productName, updateRepo } from '../../shared/branding'

let downloadCancelToken: CancelTokenSource | null = null

// Release assets are named after the product name with spaces collapsed to
// dots — must stay in sync with electron-builder.config.cjs (assetProductName).
const assetProduct = productName.replace(/ /g, '.')

export async function checkUpdate(): Promise<AppVersion | undefined> {
  const { 'mixed-port': mixedPort = 0 } = (await getRuntimeConfig()) ?? {}
  const url = `https://github.com/${updateRepo}/releases/latest/download/latest.yml`
...
export async function downloadAndInstallUpdate(version: string): Promise<void> {
  ...
  const baseUrl = `https://github.com/${updateRepo}/releases/download/${releaseTag}/`
  const fileMap = {
    'win32-x64': `${assetProduct}_x64-setup.exe`,
    'win32-arm64': `${assetProduct}_arm64-setup.exe`,
    'darwin-x64': `${assetProduct}_x64.pkg`,
    'darwin-arm64': `${assetProduct}_arm64.pkg`
  }
  ...
  const apiUrl = `https://api.github.com/repos/${updateRepo}/releases/tags/${releaseTag}`
